...
function setupSubscriptionManagement() {
  // Elements
  const activeSubscriptionView = document.getElementById('activeSubscriptionView');
  const noSubscriptionView = document.getElementById('noSubscriptionView');
  const pricingPlansSection = document.getElementById('pricingPlansSection');
  const showPricingPlansBtn = document.getElementById('showPricingPlansBtn');
  const generateQuoteBtn = document.getElementById('generateQuoteBtn');
  const subscriptionPricingToggle = document.getElementById('subscriptionPricingToggle');
  ...
  if (subscriptionTab) {
    ...
    demoToggleBtn.addEventListener('click', function() {
      ...
      localStorage.setItem('hibp_has_subscription', !currentState);
  ...
  if (showPricingPlansBtn && pricingPlansSection) {
    showPricingPlansBtn.addEventListener('click', function() {
  ...
  if (selectPlanButtons.length > 0) {
    selectPlanButtons.forEach(button => {
      button.addEventListener('click', function() {
        ...
        showReturnNotification(
          'success',
          `You have selected the ${planName} plan. Redirecting to checkout...`
        );
  ...
  if (generateQuoteBtn) {
    generateQuoteBtn.addEventListener('click', function() {
      ...
      showReturnNotification(
        'info',
        'Generating quote for your subscription. You will receive it via email shortly.'
      );
    });
  }
  ...
  if (manageBillingBtn) {
    manageBillingBtn.addEventListener('click', function(e) {
      ...
      showReturnNotification(
        'info',
        'Redirecting to the billing management portal...'
      );
    });
  }
...
function setupApiKeyGeneration() {
  ...
  const apiKeyGenerateSection = document.getElementById('apiKeyGenerateSection');
  const apiKeyDisplaySection = document.getElementById('apiKeyDisplaySection');
  ...
  if (copyApiKeyBtn) {
    new bootstrap.Tooltip(copyApiKeyBtn);
  }
  ...
  if (copyApiKeyBtn) {
    copyApiKeyBtn.addEventListener('click', function() {
      ...
      navigator.clipboard.writeText(apiKey).then(() => {
        ...
        const tooltip = bootstrap.Tooltip.getInstance(copyApiKeyBtn);
